...
"""
Object detection function. This function will apply on each frame of the live webcam
Args:
...
def detect(frame, net, transform):
    ...
    for cls in range(detections.size(1)):
        ...
        while detections[0, cls, occurence, 0] >= threshold:
            ...
            cv2.putText(frame, labelmap[cls-1] + " " + str(round(detections[0, cls, occurence, 0],2)), (x0, y0), cv2.FONT_HERSHEY_SIMPLEX, 2, (255,255,255))
            occurence += 1
    return frame
...
while True:
    ...
    frame_output = detect(frame_input, net.eval(), transform)
